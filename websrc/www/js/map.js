...
var MapUtil = {
    ...
    controls: {
        ...
        legendControl: function(opt_options) {
            ...
            legendHtml += '<li class=""><svg height="15" width="19" class="dotText"><circle cx="9" cy="8" r="6" stroke="green" stroke-width="2" fill="rgb(0,255,0)"></circle></svg> 교차로(설치량 충족)</li>';
            legendHtml += '<li class=""><svg height="15" width="19" class="dotText"><circle cx="9" cy="8" r="6" stroke="red" stroke-width="2" fill="rgb(255,0,0)"></circle></svg> 교차로(설치량 미충족)</li>';
            legendHtml += '</ul>';
    ...
    openDetailSigCd:function(layerID, sn, sigCd){
        ...
        switch (layerID) {
            case DATA_TYPE.SPPN:
                ...
                break;
            case DATA_TYPE.AOT:
                var objKndCd = sn.substr(0,5);
                switch (objKndCd) {
                    case "OBJ01"://둔치주차장 정보조회
                        url     = pages.detail_spotAotRiverPk;
                        header  = "둔치주차장";
                        break;
                    case "OBJ02"://지진옥외대피소 정보조회
                        url     = pages.detail_spotAotEqOut;
                        header  = "지진옥외대피소";
                        break;
                    case "OBJ04"://택시승강장 정보조회
                        url     = pages.detail_spotAotTaxiSt;
                        header  = "택시승강장";
                        break;
        ...
        $(detailTaget).load(url.link(), function() {
            MapUtil.setDetail(layerID, sn, sigCd);
            MapUtil.handler.photoToggleHandler();
            MapUtil.handler.takePhotoHandler();
            MapUtil.handler.delPhotoHandler();
            MapUtil.handler.dataPopupCloserHandler();
    ...
    setList: function(type){
        switch (type) {
            ...
            case "sppnList":
                ...
                makeOptSelectBox("fcltylcCdSel","FCLTYLC_CD","","전체","");
    ...
    setDetail: function(layerID, sn, sigCd) {
        var codeList
        isPopState = "off";
        toggleDetailView();
        switch (layerID) {
            ...
            case DATA_TYPE.BULD:
                ...
                break;
            case DATA_TYPE.SPPN:
                // var link = URLs.selectSppnPanInfo;
                // try{
                    MapUtil.setValuesSppn(layerID, sn, sigCd);
                // }catch(error){
                //     util.toast(msg.checkSetObjectError,"error");
                //     util.dismissProgress();
                // }
            ...
            case DATA_TYPE.AOT:
                // var link = URLs.selectSppnPanInfo;
                // try{
                    MapUtil.setValuesAot(layerID, sn, sigCd);
                // }catch(error){
                //     util.toast(msg.checkSetObjectError,"error");
                //     util.dismissProgress();
                // }
            ...
            case DATA_TYPE.CRSRDP_P:
                    ...
                    MapUtil.setValuesCrsrdp(layerID, sn, sigCd);
    ...
    setValuesSppn:function(layerID, spoNoSeq, sigCd) {
        var link = URLs.selectSppnPanInfo;
        var url = URLs.postURL(link, { "spoNoSeq": spoNoSeq, "sigCd": sigCd, "workId": app.info.opeId });
        util.showProgress();
        util.postAJAX({}, url).then(
            function(context, rCode, results) {
                //통신오류처리
                ...
                if(layerID == DATA_TYPE.SPPN){
                    ...
                    //시설물 분류
                    $("#fcltylc").html(data.fcltylc);

                    //설치기관 명칭
                    $("#instt").html(data.instt);
                    //설치기관 사업자등록번호
                    $("#insttNum").html(data.insttNum);
                    //설치기관 주소
                    $("#insttAddr").html(data.insttAddr);
                    //설치일자
                    $("#insttDe").html(data.insttDe);
                    //검증일자
                    $("#vrifyDe").html(data.vrifyDe);
                    ...
                    //높이
                    $("#ellipseSolid").html(data.ellipseSolid);
                    ...
                    var rcRslt = data.rcRslt; //점검결과
                    try{
                        var rcDe = data.rcDe == null ? "-" : "{0}년{1}월{2}일".format(data.rcDe.substr(0, 4), data.rcDe.substr(4, 2), data.rcDe.substr(6, 2));
                    } catch (error) {
                       util.toast(msg.checkObject.format("점검날짜"),"error");
                    }
    ...
    setValuesAot: function(layerID, objMngNo, sigCd) {
        var link = URLs.selectSpotObjInfo;
        var objKndCd = objMngNo.substr(0,5);
        switch (objKndCd) {
            case "OBJ01"://둔치주차장 정보조회
                link = URLs.selectSpotRiverPkInfo;
                break;
            case "OBJ02"://지진옥외대피소 정보조회
                link = URLs.selectSpotEqOutInfo;
                break;
            case "OBJ04"://택시승강장 정보조회
                link = URLs.selectSpotTaxiStInfo;
                break;
        ...
        util.postAJAX({}, url).then(
            function(context, rCode, results) {
                ...
                if(layerID == DATA_TYPE.AOT){
                    
                    //사물주소일련번호
                    $("#trgSn").val(data.OBJ_MNG_NO);
                    //시군구코드
                    $("#sigCd").val(data.SIG_CD);
                    //사물종류
                    $("#trgCd").val(data.OBJ_KND_CD);

                    /** 기본정보 */
                    //지역
                    $("#sigCdNm").html(data.SIG_CD_NM);
                    //진행상태
                    $("#progrsSttCdNm").html(data.PROGRS_STT_CD_NM);
                    //사물명
                    $("#objNm").html(data.OBJ_NM);
                    //고유번호
                    $("#innb").html(data.INNB);
                    //건물여부
                    $("#buldYn").html(data.BULD_YN);
                    //관리기관
                    $("#manIns").html(data.MAN_INS);
                    //관리부서
                    $("#manDeptNm").html(data.MAN_DEPT_NM);
                    //관리자명
                    $("#mngrNm").html(data.MNGR_NM);
                    //전화번호
                    $("#mngrTelNo").html(data.MNGR_TELNO);
                    //관련지번
                    $("#jibunAdres").html(data.JIBUN_ADRES);
                    


                    /** 부가정보 */
                    //영문명
                    $("#objEngNm").html(data.OBJ_ENG_NM);

                    /** 사물주소 */
                    var objAddrList = data.objAddrList;
                    if(objAddrList.length == 0){
                        $("#objAddrTable > tbody").empty();
                        var rowHtml = '<tr class=""><td colspan="4">검색된 목록이 없습니다.</td></tr>';
                        $("#objAddrTable > tbody:last").append(rowHtml);
                    }else{
                        var rowHtml = '<tr id={0}><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td></tr>';

                        for(var i in objAddrList){
                            var ADRES          = objAddrList[i].ADRES;

                            var POINT_SE_NM    = objAddrList[i].POINT_SE_NM;
                            var MAIN_ENT_SE_NM = objAddrList[i].MAIN_ENT_SE_NM;

                            $("#objAddrTable > tbody:last").append(
                                rowHtml.format(
                                    "row"+(i+1)
                                    ,++i
                                    ,POINT_SE_NM
                    ...
                    switch (objKndCd) {
                        ...
                        case "OBJ02":
                            ...
                            $("#fcltyForm").html(data.FCLTY_FORM);
                            //시설구분
                            $("#fcltySe").html(data.FCLTY_SE);
                            //수용가능인원
                            $("#acmdPosblNmpr").html(data.ACMD_POSBL_NMPR);
                    ...
                    var rcDe = data.RC_DE; //점검일자
                    try{
                        var rcDeTxt = rcDe == null ? "-" : "{0}년{1}월{2}일".format(rcDe.substr(0, 4), rcDe.substr(4, 2), rcDe.substr(6, 2));
                    } catch (error) {
                       util.toast(msg.checkObject.format("점검날짜"),"error");
                    }
    ...
    setValuesCrsrdp: function(layerID, crsrdSn, sigCd){
        var link = URLs.selectCrsrdp;
        var url = URLs.postURL(link, { "crsrdSn": crsrdSn, "sigCd": sigCd, "workId": app.info.opeId});
        util.showProgress();
        util.postAJAX({}, url).then(
            function(context, rCode, results) {
                //통신오류처리
                ...
                // 제목
                var title = "<span class='label'>[{0}] {1}<span>".format('교차로',data.KOR_CRSRD);
                $(".title").append(title);
                
                //시군구코드
                var SIG_CD = data.SIG_CD;
                $("#sigCd").val(SIG_CD);

                // 교차로일련번호
                var CRSRD_SN = data.CRSRD_SN;
                $("#crsrdSn").html(CRSRD_SN);

                // 교차로_한글
                var KOR_CRSRD = data.KOR_CRSRD;
                $("#korCrsrd").html(KOR_CRSRD);

                // 교차로유형
                var CRSRD_TYCD = data.CRSRD_TYCD;
                var CRSRD_TYCD_LBL = data.CRSRD_TYCD_LBL;
                makeOptSelectBox('crsrdTycd','CRSRD_TYCD');
                $("#crsrdTycd").val(CRSRD_TYCD);
                $("#crsrdTycdLbl").html(CRSRD_TYCD_LBL);
                

                // 보행자용 소요량
                var REQAMT_P = data.REQAMT_P;
                for(var i = 0 ; i < 9; i ++){
                    var optionFormat = "<option value='{0}'>{1}</option>";
                    $("#reqamtP").append(optionFormat.format(i,i));
                }
                $("#reqamtP").val(REQAMT_P);

                var REQAMT_P_H = data.REQAMT_P_H;
                $("#reqamtP_h").html(REQAMT_P_H);

                // 보행자용 소요량 변경사유
                var REQAMT_PRN = data.REQAMT_PRN;
                $("#reqamtPrn").val(REQAMT_PRN);

                // 보행자용 설치량
                var BASELBL_P = data.BASELBL_P;
                $("#baseLbl_P").html(BASELBL_P);

                // 보행자용 추가 설치 필요량
                var baseLbl_P_cnt = REQAMT_P_H - BASELBL_P;
                if(baseLbl_P_cnt < 0 ){
                    baseLbl_P_cnt = 0;
                }
                $("#baseLbl_P_cnt").html(baseLbl_P_cnt);

                // 차량용 소요량
                var REQAMT_C = data.REQAMT_C;
                for(var i = 0 ; i < 9; i ++){
                    var optionFormat = "<option value='{0}'>{1}</option>";
                    $("#reqamtC").append(optionFormat.format(i,i));
                }
                $("#reqamtC").val(REQAMT_C);

                var REQAMT_C_H = data.REQAMT_C_H;
                $("#reqamtC_h").html(REQAMT_C_H);

                // 차량용 소요량 변경사유
                var REQAMT_CRN = data.REQAMT_CRN;
                $("#reqamtCrn").val(REQAMT_CRN);

                // 차량용 설치량
                var BASELBL_C = data.BASELBL_C;
                $("#baseLbl_C").html(BASELBL_C);

                // 차량용 추가 설치 필요량
                var baseLbl_C_cnt = REQAMT_C_H - BASELBL_C;
                if(baseLbl_C_cnt < 0 ){
                    baseLbl_C_cnt = 0;
                }
                $("#baseLbl_C_cnt").html(baseLbl_C_cnt);

                // 교차로구분
                var CRSRD_SE = data.CRSRD_SE;
                var CRSRD_SE_LBL = data.CRSRD_SE_LBL;
                makeOptSelectBox('crsrdSe','CRSRD_SE');
                $("#crsrdSe").val(CRSRD_SE);
                $("#crsrdSeLbl").html(CRSRD_SE_LBL);
                

                // 교차로 사용대상 여부
                var CRS_USE_YN = data.CRS_USE_YN;
                makeOptSelectBox('crsUseYn','CRS_USE_YN');
                $("#crsUseYn").val(CRS_USE_YN);
                // var CRS_USE_YN_LBL = data.CRS_USE_YN_LBL;
                // $("#crsUseYnLbl").html(CRS_USE_YN_LBL);
                
                // //변경사유
                // customSelectBox("notUseCd","NOT_USE_CD","03","0","2");

                //관련 도로구간
                var relList = data.relList;

                for(var i in relList){
                    var rowHtml = '<tr id={0}><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td></tr>';
                    
                    //도로구간일련번호
                    var RDS_MAN_NO = relList[i].RDS_MAN_NO;
                    //도로명
                    var RN = relList[i].RN;
                    //도로명코드
                    var RN_CD = relList[i].RN_CD;
                    //도로구간시군구명
                    var SIG_KOR_NM = relList[i].SIG_KOR_NM;

                    $("#relTable > tbody:last").append(
                        rowHtml.format(
                            "row" + i
                            ,RDS_MAN_NO
                            ,RN
...
var mapInit = function(mapId, pos) {
    ...
    //교차로_보행자용
    var lyr_tlv_sprd_crsrdp_p = getFeatureLayer({
        title: "교차로_보행자용",
        typeName: "tlv_sprd_crsrdp_p",
        dataType: DATA_TYPE.CRSRDP_P,
        style: {
    ...
    //교차로_차량용
    var lyr_tlv_sprd_crsrdp_c = getFeatureLayer({
        title: "교차로_차량용",
        typeName: "tlv_sprd_crsrdp_c",
        dataType: DATA_TYPE.CRSRDP_C,
        style: {
...
//    map.getViewport().addEventListener('click', function(event) {
        ...
        map.forEachFeatureAtPixel(event.pixel, function(feature, layer) {
            ...
            //간략정보 조회
            selectFeatureInfo(features,featureIndex,layerID);
...
var getFeatureLayer = function(options) {
            ...
            util.postAJAX('', urldata, true)
                .then(function(context, rCode, results) {
                    ...
                    for (var i = 0; features.length > i; i++) {
                        ...
                        // else if(dataType == DATA_TYPE.CRSRDP_C || ){
                        //     features[i].setGeometry(features[i].getGeometry().getInteriorPoint());
                        // }
    ...
    // var type = features.getGeometry().getType();
    // if(type == "Polygon"){
    //     features.setGeometry(features.getGeometry().getInteriorPoint());
    // }
...
function openDetailPopupCallSigCd(index, layer, sn, sigCd) {
    trgSnGlobal = sn;
    featureIndex = index;
    MapUtil.openDetailSigCd(layer, sn, sigCd);
...
//지도 간략정보 표시
function selectFeatureInfo(features,popIndex,layerID){
    try {
        ...
        features.forEach(function(feature, index) {
            ...
            switch (layerID) {
                ...
                case DATA_TYPE.SPPN:
                    ...
                    buttonHtml = "";
                    //지점번호_시설물일련번호
                    var SPO_NO_SEQ = feature.get("SPO_NO_SEQ");
                    //지점번호코드
                    ...
                    resultHtml += popDiv.format("", "openDetailPopupCallSigCd(" + index + ",'" + layerID + "','" + SPO_NO_SEQ + "','"+ SIG_CD +"')", strHtml);

                    //도로시설물 공간정보
                    var geom = feature.getGeometry().getCoordinates();

                    buttonHtml += buttonForm.format("more", "openDetailPopupCallSigCd(" + index + ",'" + layerID + "','" + SPO_NO_SEQ + "','"+ SIG_CD +"')", "image/more.png", "더보기");
                ...
                case DATA_TYPE.AOT:
                        ...
                        var gbn = commonP.format("gbn", "[{0}]".format(titleText));
                        var title = commonP.format("localTitle",objNm);

                        strHtml += gbn
                        strHtml += title
                        // strHtml += commonP.format("",DTOR_NM);

                        resultHtml += popDiv.format("", "openDetailPopupCallSigCd(" + index + ",'" + layerID + "','" + objMngNo + "','"+ SIG_CD +"')", strHtml);

                        // 공간정보
                        var geom = feature.getGeometry().getCoordinates();

                        buttonHtml += buttonForm.format("more", "openDetailPopupCallSigCd(" + index + ",'" + layerID + "','" + objMngNo + "','"+ SIG_CD +"')", "image/more.png", "더보기");
                ...
                case DATA_TYPE.CRSRDP_P:
                    ...
                    //교차로일련번호
                    var CRSRD_SN = feature.get("CRSRD_SN");
                    var BASELBL = feature.get("BASELBL");
                    var KOR_CRSRD = feature.get("KOR_CRSRD");
                    var CRSRD_TYCD = feature.get("CRSRD_TYCD");
                    var SIG_CD = feature.get("SIG_CD");
                    // var REQAMT_P = feature.get("REQAMT_P");
                    // var REQAMT_C = feature.get("REQAMT_C");

                    var CRSRD_TYCD_LBL = app.codeMaster[CODE_GROUP["CRSRD_TYCD"]][CRSRD_TYCD];
                    

                    // var text0 = commonSpan.format("info", "라벨 : " + BASELBL);
                    var text1 = commonSpan.format("info", "[교차로]" + KOR_CRSRD);
                    var text2 = commonSpan.format("info", "유형 : " + CRSRD_TYCD_LBL);
                    
                    var REQAMT_P = app.codeMaster[CODE_GROUP["REQAMT_P"]][CRSRD_TYCD];
                    var text3 = commonSpan.format("info", "보행자용 도로명판 소요량 : " + REQAMT_P);
                    var text4 = commonSpan.format("info", "보행자용 도로명판 설치량 : " + BASELBL);

                    if(layerID == DATA_TYPE.CRSRDP_C){
                        var REQAMT_C = app.codeMaster[CODE_GROUP["REQAMT_C"]][CRSRD_TYCD];
                        text3 = commonSpan.format("info", "차량용 도로명판 소요량 : " + REQAMT_C);
                        text4 = commonSpan.format("info", "차량용 도로명판 설치량 : " + BASELBL);
                    }
                    // var text3 = commonSpan.format("info", "차량용 도로명판 소요량 : " + REQAMT_C);

                    strHtml += commonP.format("", text1);
                    strHtml += commonP.format("", text2);
                    strHtml += commonP.format("", text3);
                    strHtml += commonP.format("", text4);

                    resultHtml += popDiv.format("", "openDetailPopupCallSigCd(" + index + ",'" + layerID + "'," + CRSRD_SN + "," + SIG_CD +")", strHtml);

                    //도로시설물 공간정보
                    var geom = feature.getGeometry().getCoordinates();

                    buttonHtml += buttonForm.format("more", "openDetailPopupCallSigCd(" + index + ",'" + layerID + "'," + CRSRD_SN + "," + SIG_CD +")", "image/more.png", "더보기");
